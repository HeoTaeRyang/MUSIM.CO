...
  };
  ...
  const handleReward = async () => {
    ...
    if (isProcessingReward) { // 중복 클릭 방지
      return;
    }
    ...
    try {
      const response = await axios.post('/daily_mission/reward', { user_id: userId });
      ...
      if (response.data.success) {
        ...
        setRewardMessage(`보상 실패: ${response.data.error || '알 수 없는 오류'}`);
      }
    } catch (error) {
      console.error("보상 API 호출 오류:", error);
      if (axios.isAxiosError(error) && error.response) {
        setRewardMessage(`보상 오류: ${error.response.data.error || error.message}`);
      } else {
        setRewardMessage(`보상 중 오류 발생: ${error instanceof Error ? error.message : String(error)}`);
      }
    } finally {
